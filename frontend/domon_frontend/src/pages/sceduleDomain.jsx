...
function SceduleFunctionalities() {
  ...
  const handleDomainSchedule = async () => {
    ...
    console.log(payload)
    try {
      const response = await fetch(config.INSERT_SCEDULE_DOMAIN_URL, {
        method: "POST", headers: {
          "Content-Type": "application/json", "Authorization": `Bearer ${cookieData.access_token}`// Add Bearer token here
    ...
    <p>{apiResponse.message}</p>
                ...
                <div className="bg-white rounded-lg shadow-xl p-6 w-96 relative z-50">
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 text-white rounded-md font-medium ${(showModal ? scheduledDomains : expirationDomains).trim() === ""
        ...
        <section className="p-10 bg-white text-gray-800">
          ...
          <div>
            ...
            {Object.entries(apiResponse).length > 1 && (
              <div
                className={`pt-3 mt-5 px-4 py-3 rounded-md shadow-md text-sm  text-center font-medium ${apiResponse.status ? "bg-red-100 text-red-700 border border-red-400" : "bg-green-100 text-green-700 border border-green-400"
                  }`}
              >
                <p>{apiResponse.message}</p>
              </div>
